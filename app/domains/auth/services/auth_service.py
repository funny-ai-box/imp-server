...
from app.core.exceptions import APIException, AuthenticationException, ValidationException
from app.core.status_codes import (
    AUTH_FAILED, TOKEN_EXPIRED, INVALID_TOKEN, PARAMETER_ERROR,
    ACCOUNT_LOCKED, PASSWORD_ERROR, USER_NOT_FOUND, USER_ALREADY_EXISTS
)
...
class AuthService:
    ...
    def __init__(self, auth_repository: AuthRepository, user_repository: Optional[UserRepository] = None):
        """初始化认证服务
    ...
    def register_with_phone_password(self, phone: str, encrypted_password: str, username: Optional[str] = None) -> Dict[str, Any]:
        """手机号密码注册
        ...
        try:
            ...
            user = self.auth_repo.register_user(
                ...
                username=username or phone  # 如果未提供用户名，使用手机号
            )
    ...
    def login_with_phone_password(self, phone: str, encrypted_password: str, ip_address: Optional[str] = None, user_agent: Optional[str] = None) -> Dict[str, Any]:
        """手机号密码登录
        ...
        try:
            ...
            if not user:
                ...
                # 记录登录失败
                if self.user_repo:
                    # 这里使用一个临时用户ID，因为用户不存在
                    self.auth_repo.record_login(
                        user_id=0,
                ...
                raise AuthenticationException("手机号或密码不正确", AUTH_FAILED)
            ...
            if not check_password_hash(user.password_hash, password):
                ...
                raise AuthenticationException("手机号或密码不正确", PASSWORD_ERROR)
            ...
            raise AuthenticationException("登录过程中发生错误", AUTH_FAILED)
