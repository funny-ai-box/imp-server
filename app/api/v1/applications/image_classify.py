...
@image_classify_bp.route("/classification/<int:classification_id>", methods=["GET"])
@auth_required
def get_classification(classification_id):
    ...
    try:
        ...
        # 获取分类记录
        classification = classify_service.get_classification(classification_id, user_id)

        return success_response(classification, "获取分类记录成功")
    except Exception as e:
        # 记录未知错误
        if not isinstance(e, ValidationException):
            logger.error(
                f"Error getting classification: {str(e)}\n{traceback.format_exc()}"
            )
        raise


@image_classify_bp.route("/classification/<int:classification_id>/rate", methods=["POST"])
@auth_required
def rate_classification(classification_id):
    ...
    try:
        ...
        # 提交评分
        classification = classify_service.rate_classification(
        ...
        return success_response(classification, "评分成功")
    except Exception as e:
        # 记录未知错误
        if not isinstance(e, ValidationException):
            logger.error(f"Error rating classification: {str(e)}\n{traceback.format_exc()}")
        raise


@image_classify_bp.route("/delete_classification", methods=["POST"])
@auth_required
def delete_classification():
    ...
    try:
        ...
        # 删除分类记录
        result = classify_service.delete_classification(classification_id, user_id)

        return success_response({"success": result}, "删除分类记录成功")
    except Exception as e:
        # 记录未知错误
        if not isinstance(e, ValidationException):
            logger.error(
                f"Error deleting classification: {str(e)}\n{traceback.format_exc()}"
            )
        raise


@image_classify_bp.route("/statistics", methods=["GET"])
@auth_required
def get_statistics():
    """获取图片分类统计数据"""
    try:
        ...
        # 获取统计数据
        statistics = classify_service.get_statistics(user_id, days)
